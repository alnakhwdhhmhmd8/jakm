...
CurrentChatId = data and data.chat_id
...
send(data.actor_user_id,0,'\nليه تركت قروبك\nName ▸ ['..(UserInfo.first_name or '؟')..'](tg://user?id='..data.actor_user_id..')\nId ▸ '..data.actor_user_id..'\nTime ▸ '..os.date("%p %I:%M:%S")..'\nDate ▸ '..os.date("%Y/%m/%d")..'\nLink ▸ ['..(Get_Chat and Get_Chat.title or 'القروب')..']('..safe_invite(Info_Chats)..')',"md",true) 
send(data.chat_id,0,'\nمالك القروب غادر 😨\nName ▸ ['..(UserInfo.first_name or '؟')..'](tg://user?id='..data.actor_user_id..')\nId ▸ '..data.actor_user_id..'\nTime ▸ '..os.date("%p %I:%M:%S")..'\nDate ▸ '..os.date("%Y/%m/%d")..'\n️',"md",true) 
end
...
local Info_Chats = bot.getSupergroupFullInfo(data.chat_id)
local UserInfo = bot.getUser(data.actor_user_id)
local _admin_leave_markup = bot.replyMarkup{type='inline',data={{{text='🔗 '..Get_Chat.title, url=safe_invite(Info_Chats)},{text='👤 '..(UserInfo.first_name or 'مالك'), url='tg://user?id='..data.actor_user_id},},}}
send(data.actor_user_id,0,'\nليه تركت رتبتك ( مشرف ) وغادرت\nName ▸ ['..(UserInfo.first_name or '؟')..'](tg://user?id='..data.actor_user_id..')\nId ▸ '..data.actor_user_id..'\nTime ▸ '..os.date("%p %I:%M:%S")..'\nDate ▸ '..os.date("%Y/%m/%d")..'\nLink ▸ ['..(Get_Chat and Get_Chat.title or 'القروب')..']('..safe_invite(Info_Chats)..')\n️',"md",true,false,false,false,_admin_leave_markup) 
send(data.chat_id,0,'\nمشرف غادر القروب 😞\nName ▸ ['..UserInfo.first_name..'](tg://user?id='..data.actor_user_id..')\nId ▸ '..data.actor_user_id..'\nTime ▸ '..os.date("%p %I:%M:%S")..'\nDate ▸ '..os.date("%Y/%m/%d")..'\n️',"md",true) 
end
end
if (data.new_chat_member.status.luatele == "chatMemberStatusMember" or data.new_chat_member.status.luatele == "chatMemberStatusAdministrator") and 
   (data.old_chat_member.status.luatele == "chatMemberStatusLeft" or data.old_chat_member.status.luatele == "chatMemberStatusBanned") and
   data.actor_user_id ~= UserCheck then
Redis:setex(TheMero.."Mero:AddedBy:"..data.chat_id..":"..UserCheck, 86400*30, data.actor_user_id)
end
...
if (data.new_chat_member.status.luatele == "chatMemberStatusLeft" or data.new_chat_member.status.luatele == "chatMemberStatusBanned") and
   (data.old_chat_member.status.luatele == "chatMemberStatusMember" or data.old_chat_member.status.luatele == "chatMemberStatusRestricted") and
   data.actor_user_id ~= UserCheck then
if Redis:get(TheMero.."Mero:AntiKick"..data.chat_id) then
local _kickKey = TheMero.."AntiKick:count:"..data.chat_id..":"..data.actor_user_id
local _kickCount = tonumber(Redis:get(_kickKey) or 0) + 1
Redis:setex(_kickKey, 600, _kickCount)
if _kickCount >= 5 then
local _actorChatMem = bot.getChatMember(data.chat_id, data.actor_user_id)
if _actorChatMem and _actorChatMem.status and _actorChatMem.status.luatele == "chatMemberStatusAdministrator" then
bot.setChatMemberStatus(data.chat_id, data.actor_user_id, 'member')
local _actorInfo = bot.getUser(data.actor_user_id)
local _actorName = (_actorInfo and _actorInfo.first_name) or 'مشرف'
send(data.chat_id,0,'⇜ *تحذير — منع التصفية*\n['..(_actorName)..'](tg://user?id='..data.actor_user_id..') قام بتصفية '..(_kickCount)..' أشخاص في 10 دقائق\nتم سحب رتبته تلقائياً',"md",true)
Redis:del(_kickKey)
end
end
end
...
local _new_rights = data.new_chat_member.status and data.new_chat_member.status.rights
local _old_rights = data.old_chat_member.status and data.old_chat_member.status.rights
-- للقنوات: أي رتبة إدارية تُفعّل البوت | للمجموعات: يُشترط can_delete_messages
local _ch_pre = bot.getChat(data.chat_id)
local _is_ch_pre = _ch_pre and _ch_pre.type and _ch_pre.type.is_channel == true
local _new_is_admin = data.new_chat_member.status and data.new_chat_member.status.luatele == "chatMemberStatusAdministrator"
local _old_is_admin = data.old_chat_member.status and data.old_chat_member.status.luatele == "chatMemberStatusAdministrator"
local _activate_ok
if _is_ch_pre then
_activate_ok = _new_is_admin or _old_is_admin
else
_activate_ok = (_new_rights and _new_rights.can_delete_messages == true) or (_old_rights and _old_rights.can_delete_messages == true)
end
if _activate_ok then
...
-- للمجموعات فقط: فحص الحد الأدنى للأعضاء (القنوات تُعفى من هذا الشرط)
if not _is_ch_pre then
local Info_Chats = bot.getSupergroupFullInfo(chat_id) ---check if count is true
if tonumber((Info_Chats and Info_Chats.member_count or 0)) < tonumber((Redis:get(TheMero..'Mero:Num:Add:Bot') or 0)) then
...
-- فحص اذا كانت قناة
local is_channel_chat = Get_Chat and Get_Chat.type and Get_Chat.type.is_channel == true
local chat_label = is_channel_chat and 'قناة' or 'قروب'
-- جلب معلومات المروج بأمان (في القنوات قد يكون who_promot = 0)
local UserInfo = {}
if who_promot and tonumber(who_promot) and tonumber(who_promot) > 0 then
UserInfo = bot.getUser(who_promot) or {}
end
local promoter_name = UserInfo.first_name or 'مدير القناة'
local promoter_id = (who_promot and tonumber(who_promot) and tonumber(who_promot) > 0) and who_promot or Sudo_Id
----
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
{text = Get_Chat.title, url = safe_invite(Info_Chats)}, 
},
}
}
send(Sudo_Id,0,'\n⇜ تم تفعيل '..chat_label..' جديد'..'\n⇜ من قبل : ['..promoter_name..'](tg://user?id='..promoter_id..')\n⇜ معلومات ال'..chat_label..' :\n⇜ عدد الاعضاء : '..(Info_Chats and Info_Chats.member_count or 0)..'\n⇜ الرابط : ['..(Get_Chat and Get_Chat.title or chat_label)..']('..safe_invite(Info_Chats)..')\n⇜ ايدي ال'..chat_label..' : `'..chat_id..'`',"md",true, false, false, false, reply_markup)
-----
...
-- تسجيل القناة كقناة لتمييزها لاحقاً
if is_channel_chat then
Redis:set(TheMero.."Mero:IsChannel:"..chat_id, "true")
end
-----
-- جلب قائمة الادمنيه بأمان (قد يختلف في القنوات)
pcall(function()
local Info_Members = bot.getSupergroupMembers(chat_id, "Administrators", "*", 0, 200)
if Info_Members and Info_Members.members then
...
-----
local txt = '⇜ *تم تفعيل ال'..chat_label..' بنجاح* 🎉\n\n'..
'📌 *ال'..chat_label..':* ['..(Get_Chat and Get_Chat.title or chat_label)..']('..safe_invite(Info_Chats)..')\n'..
'👥 *الأعضاء:* '..(Info_Chats and Info_Chats.member_count or 0)..'\n\n'..
'⇜ اكتب *م1* لعرض قائمة الأوامر'
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
{text = '⚙️ اعدادات الحماية', data=promoter_id..'/'.. 'NoNextSeting'},
},
{
{text = '📋 قائمة الاوامر', data=promoter_id..'/helpall'},
},
}
}
...
if data and data.luatele and data.luatele == "updateChatJoinRequest" then
local join_chat_id = data.chat_id
local join_user_id = data.user_id
if join_chat_id and join_user_id then
if Redis:get(TheMero.."Mero:AutoAcceptJoin"..join_chat_id) then
pcall(function()
bot.processChatJoinRequest(join_chat_id, join_user_id, true)
end)
local UserInfo = bot.getUser(join_user_id)
local userName = (UserInfo and UserInfo.first_name) or 'عضو'
send(join_chat_id,0,'⇜ *تم قبول طلب الانضمام تلقائياً*\n⇜ العضو : ['..userName..'](tg://user?id='..join_user_id..')',"md",true)
elseif Redis:get(TheMero.."Mero:AutoDeclineJoin"..join_chat_id) then
pcall(function()
bot.processChatJoinRequest(join_chat_id, join_user_id, false)
end)
end
end
end
...
if not inget or inget == "" then
https.request("https://api.telegram.org/bot"..Token..'/answerCallbackQuery?callback_query_id='..data.id..'&text='..URL.escape('𖦹 الهمسة انتهت صلاحيتها 𖦹')..'&show_alert=true')
else
...
if tonumber(UserId_Info.id) == tonumber(data.sender_user_id) then
local nope = {{type = 'article', id = '1', title = '⚠️ لا تستطيع ارسال همسة لنفسك', input_message_content = {message_text = '⚠️ لا يمكن ارسال همسة لنفسك', parse_mode = 'Markdown'}}}
https.request("https://api.telegram.org/bot"..Token..'/answerInlineQuery?inline_query_id='..data.id..'&results='..JSON.encode(nope))
return false
end
...
local msg_user_send_id = (msg.sender_id and msg.sender_id.user_id) or 0
local msg_id = msg.id
local text = nil
...
-- [V7] كشف القناة عندما يُظهر المشرف اسمه الحقيقي (إظهار المشرفين)
-- الصلاحيات تُحدَّد لاحقاً في File_Bot_Run (Mero.lua) عبر getChatMember
if msg.sender_id.luatele == "messageSenderUser" and msg.sender_id.user_id then
  local _ch_key_user = Redis:get(TheMero.."Mero:IsChannel:"..msg.chat_id)
  local _is_ch_user  = _ch_key_user == "true"
  -- إذا لم تكن القناة في Redis نتحقق من API (أول رسالة / قناة جديدة)
  if not _is_ch_user then
    local _ok_gc, _gc = pcall(bot.getChat, msg.chat_id)
    if _ok_gc and _gc and _gc.type and _gc.type.is_channel == true then
      Redis:set(TheMero.."Mero:IsChannel:"..msg.chat_id, "true")
      _is_ch_user = true
    end
  end
  if _is_ch_user then
    msg.IsChannel = true
    -- ملاحظة: الصلاحيات ستُحدَّد في File_Bot_Run عبر getChatMember
  end
end
if msg.sender_id.luatele == "messageSenderChat" then
-- كشف اذا كانت الرسالة من قناة (القناة ترسل باسمها = sender_id.chat_id == chat_id)
if msg.sender_id.chat_id == msg.chat_id then
-- نتحقق من Redis أولاً (أسرع وأكثر موثوقية)
local _redis_is_ch = Redis:get(TheMero.."Mero:IsChannel:"..msg.chat_id)
if _redis_is_ch == "true" then
-- قناة مؤكدة من Redis — نكمل المعالجة
msg.sender_id.user_id = msg.chat_id
msg.IsChannel = true
else
-- Redis لا يعرفها — نتحقق من API كـ fallback
local _ch_info = bot.getChat(msg.chat_id)
if _ch_info and _ch_info.type and _ch_info.type.is_channel == true then
-- قناة مكتشفة من API — نحفظها في Redis ونكمل
Redis:set(TheMero.."Mero:IsChannel:"..msg.chat_id, "true")
msg.sender_id.user_id = msg.chat_id
msg.IsChannel = true
else
-- ادمن مجهول الهوية في مجموعة (ليس قناة)
...
-- رسالة من قناة مرتبطة ترسل داخل مجموعة (sender != chat)
if Redis:sismember(TheMero.."Mero:SilentGroup:Group"..msg_chat_id,msg.sender_id.chat_id) then
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
end
if Redis:get(TheMero.."Mero:Lock:SenderChat"..msg_chat_id) then
...
if msg.IsChannel and msg.sender_id.luatele == "messageSenderChat" then
-- القناة المجهولة (النشر باسم القناة): أعلى صلاحيات للمالك
msg.The_Controller = 44
msg.Name_Controller = Redis:get(TheMero.."Mero:PresidentQ:Group:Reply"..msg.chat_id) or 'مالك القناة'
...
elseif msg.IsChannel and msg.sender_id.luatele == "messageSenderUser" then
-- مستخدم حقيقي في القناة (إظهار المشرفين) — الصلاحيات ستُحدَّد في File_Bot_Run
-- لا نضبط The_Controller هنا لضمان أن Mero.lua يُشغّل getChatMember
do end
...
-- تتبع الميديا الواردة من المستخدمين لأوامر التنظيف
if msg.content.sticker then
  Redis:sadd(TheMero.."Mero:all:sticker"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
if msg.content.video_note or msg.content.video then
  Redis:sadd(TheMero.."Mero:all:video"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
if msg.content.document then
  Redis:sadd(TheMero.."Mero:all:docu"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
if msg.content.audio then
  Redis:sadd(TheMero.."Mero:all:audio"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
if msg.content.voice_note then
  Redis:sadd(TheMero.."Mero:all:voice"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
if msg.content.photo then
  Redis:sadd(TheMero.."Mero:all:photo"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
if msg.content.animation then
  Redis:sadd(TheMero.."Mero:all:anim"..msg_chat_id, msg.id)
  Redis:sadd(TheMero.."Mero:allM"..msg_chat_id, msg.id)
end
-- فحص التنظيف التلقائي المجدول
local _now_ac = os.time()
local _due_msgs = Redis:zrangebyscore(TheMero.."autoclean:pending:"..msg_chat_id, 0, _now_ac)
if _due_msgs and type(_due_msgs) == "table" and #_due_msgs > 0 then
  for _, _mid in pairs(_due_msgs) do
    pcall(function()
      bot.deleteMessages(msg_chat_id, {tonumber(_mid)})
    end)
  end
  Redis:zremrangebyscore(TheMero.."autoclean:pending:"..msg_chat_id, 0, _now_ac)
end
RunBank(msg)
------------------------------------------------------------------
end
...
-- حماية القنوات في تحرير الرسائل
-- استخراج معرف المرسل بأمان (قد يكون قناة)
local _edit_sender_uid = (Message_Edit.sender_id and Message_Edit.sender_id.user_id) or 0
-- للقنوات: نضع IsChannel وuser_id بشكل صحيح قبل المعالجة
if Message_Edit.sender_id and Message_Edit.sender_id.luatele == "messageSenderChat" then
  local _edit_is_ch = Redis:get(TheMero.."Mero:IsChannel:"..tostring(Message_Edit.chat_id)) == "true"
  if _edit_is_ch or (Message_Edit.sender_id.chat_id == Message_Edit.chat_id) then
    _edit_sender_uid = Message_Edit.chat_id
    Message_Edit.sender_id.user_id = Message_Edit.chat_id
    Message_Edit.IsChannel = true
  end
end
if tonumber(_edit_sender_uid) == tonumber(TheMero) then
  print('This is Edit for Bot')
  return false
end
File_Bot_Run(Message_Edit,Message_Edit)
if tonumber(_edit_sender_uid) == 916165019 then
data.The_Controller = 1
elseif tonumber(_edit_sender_uid) == 916165019 then
data.The_Controller = 1
elseif The_ControllerAll(_edit_sender_uid) == true then  
data.The_Controller = 1
elseif Redis:sismember(TheMero.."Mero:DevelopersQ:Groups",_edit_sender_uid) == true then
data.The_Controller = 2
elseif Redis:sismember(TheMero.."Mero:Developers:Groups",_edit_sender_uid) == true then
data.The_Controller = 3
elseif Redis:sismember(TheMero.."Mero:TheBasicsQ:Group"..data.chat_id,_edit_sender_uid) == true then
data.The_Controller = 44
elseif Redis:sismember(TheMero.."Mero:TheBasics:Group"..data.chat_id,_edit_sender_uid) == true then
data.The_Controller = 4
elseif Redis:sismember(TheMero.."Mero:Originators:Group"..data.chat_id,_edit_sender_uid) == true then
data.The_Controller = 5
elseif Redis:sismember(TheMero.."Mero:Managers:Group"..data.chat_id,_edit_sender_uid) == true then
data.The_Controller = 6
elseif Redis:sismember(TheMero.."Mero:Addictive:Group"..data.chat_id,_edit_sender_uid) == true then
data.The_Controller = 7
elseif Redis:sismember(TheMero.."Mero:Distinguished:Group"..data.chat_id,_edit_sender_uid) == true then
data.The_Controller = 8
elseif tonumber(_edit_sender_uid) == tonumber(TheMero) then
...
-- _edit_sender_uid already set above (correct value for user or channel)
local _edit_user_ok, _edit_uinfo = pcall(bot.getUser, _edit_sender_uid)
local UserInfo = (_edit_user_ok and _edit_uinfo) or {}
local names = FlterBio(UserInfo.first_name or tostring(_edit_sender_uid))
local monsha = Redis:smembers(TheMero.."Mero:TheBasicsQ:Group"..data.chat_id) 
Redis:incr(TheMero..'Mero:Num:Message:Edit'..data.chat_id.._edit_sender_uid)
...
local UserInfo = bot.getUser(_edit_sender_uid)
local Teext = '「 ['..UserInfo.first_name..'](tg://user?id='.._edit_sender_uid..') 」\n'
return send(data.chat_id,0,Teext..'⇜ ممنوع التعديل \n ✧',"md")
end
end
end
end
if not data.Originators and _edit_sender_uid ~= tonumber(TheMero) then
...
local tecxt = ListMembers.."\n\n".."\n⇜ المستخدم ["..names.." ](tg://user?id=".._edit_sender_uid..")"..
"\n⇜ قام بتعديل الميديا"
...
if Text == 'mero_service_cmds' then
if tonumber(IdUser) == tonumber(Sudo_Id) then
local _svcList = '⇜ *قائمة الاوامر الخدمية*\n\n• عدل هذا النص بمحتوى اوامرك الفعلية'
https.request("https://api.telegram.org/bot"..Token..'/answerCallbackQuery?callback_query_id='..data.id..'&text='..URL.escape('تم إرسالها لك بالخاص ✓')..'&show_alert=false')
send(IdUser,0,_svcList,"md",true)
else
https.request("https://api.telegram.org/bot"..Token..'/answerCallbackQuery?callback_query_id='..data.id..'&text='..URL.escape('⇜ هذا الزر يخص المطور فقط')..'&show_alert=true')
end
end
chsource = Redis:get(TheMero.."Mero:Channel:Join") or ""
-- ── كشف القناة للـ Callbacks ──────────────────────────────────
local _isChannelCb = Redis:get(TheMero.."Mero:IsChannel:"..tostring(ChatId)) == "true"
if not _isChannelCb then
  local _ok_ci, _ci = pcall(bot.getChat, ChatId)
  if _ok_ci and _ci and _ci.type and _ci.type.is_channel == true then
    Redis:set(TheMero.."Mero:IsChannel:"..tostring(ChatId), "true")
    _isChannelCb = true
  end
end
-- ─────────────────────────────────────────────────────────────
if _isChannelCb then
  -- ── منطق الصلاحيات في القنوات ─────────────────────────────────
  -- الحالة أ: IdUser = 0 → المشرف يضغط الزر وهو في وضع النشر المجهول
  --           (يُنشر باسم القناة لا باسمه الشخصي)
  --           نمنحه صلاحيات مالك لأنه بالتأكيد مشرف (فقط المشرفون يرون الأزرار)
  -- الحالة ب: IdUser > 0 → مستخدم حقيقي نتحقق من رتبته بالـ API الخام
  -- ─────────────────────────────────────────────────────────────
  -- ملاحظة أمنية: في TDLib، sender_user_id=0 يُصدَر فقط لمشرف القناة المجهول
  -- (Anonymous Admin). لا يمكن للمشتركين العاديين أبداً الحصول على هذه القيمة.
  -- المصدر: TDLib docs — updateNewCallbackQuery.sender_user_id:
  --   "0 if the user is an anonymous channel admin"
  if tonumber(IdUser) == 0 then
    -- مشرف مجهول الهوية (anonymous admin) — مضمون من TDLib أنه مشرف
    IdUser = ChatId  -- نستخدم معرف القناة لأوامر مستوى القناة
    data.The_Controller = 44
    data.IsChannel      = true
    data.ControllerBot  = true
    data.DevelopersQ    = true
    data.Developers     = true
    data.MalekAsase     = true
    data.TheBasicsQ     = true
    data.TheBasics      = true
    data.Originators    = true
    data.Managers       = true
    ...
    -- مستخدم حقيقي — نتحقق من رتبته في القناة بالدالة الخام (غير الـ wrapped)
    local _cbAdmin = false
    local _raw_cm = _G_raw_getChatMember or bot.getChatMember
    local _ok_m, _mem = pcall(_raw_cm, ChatId, IdUser)
    if _ok_m and _mem and _mem.status then
      local _ms = _mem.status.luatele
      if _ms == "chatMemberStatusCreator" or _ms == "chatMemberStatusAdministrator" then
        _cbAdmin = true
      end
    end
    if _cbAdmin then
      -- مشرف حقيقي في القناة → صلاحيات مالك كاملة
      data.The_Controller = 44
      data.IsChannel      = true
      data.ControllerBot  = true
      data.DevelopersQ    = true
      ...
      -- مشترك عادي في القناة → صلاحيات محدودة
      data.The_Controller = 8
      data.IsChannel      = true
      data.Distinguished  = true
    end
...
if data.The_Controller == 1 or data.The_Controller == 2 or data.The_Controller == 3 or data.The_Controller == 44 or data.The_Controller == 4 or data.The_Controller == 5 or data.The_Controller == 6 or data.The_Controller == 7 or data.The_Controller == 8 or data.The_Controller == 9 or data.The_Controller == 10 then
data.Distinguished = true
end
if data.The_Controller == 444 or data.The_Controller == 44 then
data.MalekAsase = true
end
-- answer callback immediately to clear loading spinner on ALL buttons
https.request('https://api.telegram.org/bot'..Token..'/answerCallbackQuery?callback_query_id='..tostring(data.id))
...
if math then
local safe_math = math:gsub("[^0-9%+%-%*%/%.%(%)%^%%]","")
Noah = io.popen("echo '"..safe_math.."' | bc 2>/dev/null"):read('*a')
res = "⇜ ناتج "..math.." هو :\n "..Noah
else
...
return bot.answerCallbackQuery(data.id,"\n⇜ هييه مايمديك تستخدم الامر على ( "..Controller(ChatId,UserId[2]).." 〗 فقط .", true)
end
...
----------------------------- حماية قصوى callbacks -----------------------------
if Text and Text:match('(%d+)/prot_links(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_links(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."Mero:Lock:Link"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"del") end
local lnk = Redis:get(TheMero.."Mero:Lock:Link"..ChatId) and "✓" or "✗"
local fwd = Redis:get(TheMero.."Mero:Lock:forward"..ChatId) and "✓" or "✗"
local bots = Redis:get(TheMero.."Mero:Lock:Bot:kick"..ChatId) and "✓" or "✗"
local edt = Redis:get(TheMero.."Mero:Lock:User:Name"..ChatId) and "✓" or "✗"
local spm = Redis:get(TheMero.."Mero:Lock:Spam"..ChatId) and "✓" or "✗"
local reply_markup = bot.replyMarkup{type='inline',data={
{{text='- الروابط : '..lnk,data=uid..'/prot_links'..mid},{text='- التوجيه : '..fwd,data=uid..'/prot_fwd'..mid},},
{{text='- البوتات : '..bots,data=uid..'/prot_bots'..mid},{text='- التعديل : '..edt,data=uid..'/prot_edit'..mid},},
{{text='- السبام : '..spm,data=uid..'/prot_spam'..mid},},
{{text='- تفعيل الكل ',data=uid..'/prot_all_on'..mid},{text='- ايقاف الكل ',data=uid..'/prot_all_off'..mid},},
{{text='- اخفاء الامر ',data='/delAmr1',style='danger'},},
}}
edit(ChatId,Msg_id,"‌⇜ اعدادات الحماية القصوى :",'md',false,false,reply_markup)
end
end
if Text and Text:match('(%d+)/prot_fwd(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_fwd(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."Mero:Lock:forward"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"del") end
local lnk = Redis:get(TheMero.."Mero:Lock:Link"..ChatId) and "✓" or "✗"
local fwd = Redis:get(TheMero.."Mero:Lock:forward"..ChatId) and "✓" or "✗"
local bots = Redis:get(TheMero.."Mero:Lock:Bot:kick"..ChatId) and "✓" or "✗"
...
if Text and Text:match('(%d+)/prot_bots(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_bots(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."Mero:Lock:Bot:kick"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"del") end
local lnk = Redis:get(TheMero.."Mero:Lock:Link"..ChatId) and "✓" or "✗"
local fwd = Redis:get(TheMero.."Mero:Lock:forward"..ChatId) and "✓" or "✗"
local bots = Redis:get(TheMero.."Mero:Lock:Bot:kick"..ChatId) and "✓" or "✗"
...
if Text and Text:match('(%d+)/prot_edit(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_edit(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."Mero:Lock:User:Name"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"del") end
local lnk = Redis:get(TheMero.."Mero:Lock:Link"..ChatId) and "✓" or "✗"
local fwd = Redis:get(TheMero.."Mero:Lock:forward"..ChatId) and "✓" or "✗"
local bots = Redis:get(TheMero.."Mero:Lock:Bot:kick"..ChatId) and "✓" or "✗"
...
if Text and Text:match('(%d+)/prot_spam(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_spam(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."Mero:Lock:Spam"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"del") end
local lnk = Redis:get(TheMero.."Mero:Lock:Link"..ChatId) and "✓" or "✗"
local fwd = Redis:get(TheMero.."Mero:Lock:forward"..ChatId) and "✓" or "✗"
local bots = Redis:get(TheMero.."Mero:Lock:Bot:kick"..ChatId) and "✓" or "✗"
...
if Text and Text:match('(%d+)/prot_all_on(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_all_on(%d+)')
if tonumber(IdUser)==tonumber(uid) then
...
local reply_markup = bot.replyMarkup{type='inline',data={
{{text='- الروابط : ✓',data=uid..'/prot_links'..mid},{text='- التوجيه : ✓',data=uid..'/prot_fwd'..mid},},
{{text='- البوتات : ✓',data=uid..'/prot_bots'..mid},{text='- التعديل : ✓',data=uid..'/prot_edit'..mid},},
{{text='- السبام : ✓',data=uid..'/prot_spam'..mid},},
{{text='- تفعيل الكل ',data=uid..'/prot_all_on'..mid},{text='- ايقاف الكل ',data=uid..'/prot_all_off'..mid},},
{{text='- اخفاء الامر ',data='/delAmr1',style='danger'},},
}}
edit(ChatId,Msg_id,"‌⇜ اعدادات الحماية القصوى :",'md',false,false,reply_markup)
...
if Text and Text:match('(%d+)/prot_all_off(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/prot_all_off(%d+)')
if tonumber(IdUser)==tonumber(uid) then
...
----------------------------- ذكاء callbacks -----------------------------
if Text and Text:match('(%d+)/ai_toggle(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/ai_toggle(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."ai_disabled"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"true") end
local ai_stat = Redis:get(TheMero.."ai_disabled"..ChatId) and "لا" or "نعم"
local ai_vip_stat = Redis:get(TheMero.."ai_vip"..ChatId) and "نعم" or "لا"
local reply_markup = bot.replyMarkup{type='inline',data={
{{text='- الذكاء : '..ai_stat,data=uid..'/ai_toggle'..mid},},
{{text='- للمميزين فقط : '..ai_vip_stat,data=uid..'/ai_vip_toggle'..mid},},
{{text='- اخفاء الامر ',data='/delAmr1',style='danger'},},
}}
edit(ChatId,Msg_id,"‌⇜ اعدادات الذكاء الاصطناعي :",'md',false,false,reply_markup)
end
end
if Text and Text:match('(%d+)/ai_vip_toggle(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/ai_vip_toggle(%d+)')
if tonumber(IdUser)==tonumber(uid) then
local k = TheMero.."ai_vip"..ChatId
if Redis:get(k) then Redis:del(k) else Redis:set(k,"true") end
local ai_stat = Redis:get(TheMero.."ai_disabled"..ChatId) and "لا" or "نعم"
local ai_vip_stat = Redis:get(TheMero.."ai_vip"..ChatId) and "نعم" or "لا"
local reply_markup = bot.replyMarkup{type='inline',data={
...
----------------------------- تنظيف تلقائي callbacks -----------------------------
if Text and Text:match('(%d+)/autoclean_stop(%d+)') and data.Addictive then
local uid,mid = Text:match('(%d+)/autoclean_stop(%d+)')
if tonumber(IdUser)==tonumber(uid) then
Redis:del(TheMero.."autoclean"..ChatId)
edit(ChatId,Msg_id,"‌⇜ تم ايقاف التنظيف التلقائي",'md',false,false,false)
end
end
if Text and Text:match('(%d+)/autoclean_status(%d+)') then
bot.answerCallbackQuery(data.id,"لا يوجد إجراء على هذا الزر",true)
end
...
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'listallAddorrem', style = 'danger'},},}}
...
send(ChatId, 0, '⇜ الف مبروك لقد فاز : ['..UserInfo.first_name..'](tg://user?id='..name..') وربح 10 نقاط ', 'md')
...
data = {
{
{text = 'رجوع', data = GetUserReply[1]..'/backinthar/'..GetUserReply[2], style = 'danger'},
},
}
}
...
local toptop = "⇜ أهلين يا روح روبي هذي أوامر روبي :\n⇜ اختر نوع التوب من الازرار\n\n⇜ تتصفر اللعبة بعد ↤ "..day.."\n✧"
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
...
data = {
{
{text = '• رجوع •', data = data.sender_user_id..'/toptop', style = 'danger'}, 
},
{
{text = 'روبـي 🪄',url="t.me/FvvZv"}, 
},
...
msalm = "⇜ وسوينا لك حساب في بنك روبي\n⇜ وشحنالك ٥٠ درهم 💸 هديه\n\n⇜ رقم حسابك ↤ ❲ `"..cccall.."` ❳\n⇜ نوع البطاقة ↤ ❲ "..ccctype.." ❳\n⇜ فلوسك ↤ ❲ 50 ريال 💸 ❳\n⇜ شخصيتك ↤ "..shakseshakse.."\n⇜ دولتك ↤ ❲ 🇸🇾 ❳"
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
...
if tonumber(data.sender_user_id) == tonumber(UserId) then
edit(ChatId,Msg_id,"⇜ تم الغاء شراء اللاعب روبي سواريز ", 'md', true)
end
end
...
local safe_sc_path = UserId[2]:gsub("[^%w%-_/%.%~]","")
link = "https://soundcloud.com/"..safe_sc_path
title = UserId[1]:gsub("[^%w%-_]","_"):sub(1,60)
local nameuser = bot.getUser(IdUser)
if nameuser.first_name then
nameuser = "["..nameuser.first_name.."](tg://user?id="..nameuser.id..")"
else
...
os.execute("timeout 120 "..YTDLP_BIN.." "..YTDLP_COOKIES.." '"..link.."' --max-filesize 25M -o '"..title..".mp3'")
...
-- ─── معالج أزرار بحث الموسيقى msr: ───────────────────────────
if Text and Text:match("^(%d+):msr:(%d+):(.+)$") then
  local uid, idx, cid = Text:match("^(%d+):msr:(%d+):(.+)$")
  if tonumber(IdUser) == tonumber(uid) then
    local stored = Redis:get(TheMero.."music:res:"..uid..":"..cid..":"..idx)
    if not stored then
      return bot.answerCallbackQuery(data.id, "⇜ انتهت صلاحية نتيجة البحث", true)
    end
    local vid_id, title = stored:match("^([^|]+)|(.+)$")
    if not vid_id then
      return bot.answerCallbackQuery(data.id, "⇜ خطأ في البيانات", true)
    end
    local link = "https://youtu.be/"..vid_id:gsub("[^%w%-_]","")
    local sfname = title:gsub("[^%w%s%-]",""):gsub("%s+","_"):sub(1,55)
    if sfname == "" then sfname = "track_"..math.random(10000,99999) end
    local reply_markup = bot.replyMarkup{type='inline', data={
      {{text='🎵 تحميل صوت', data=uid..":msdl:mp3:"..idx..":"..cid},
       {text='🎬 تحميل فيديو', data=uid..":msdl:mp4:"..idx..":"..cid}},
    }}
    bot.editMessageText(ChatId, Msg_id,
      "🎵 *"..title.."*\n🔗 [يوتيوب]("..link..")\n\n_اختر صيغة التحميل ↓_",
      "md", true, false, reply_markup)
  else
    bot.answerCallbackQuery(data.id, "⇜ الأمر ليس لك", true)
  end
end

-- ─── معالج تحميل الموسيقى msdl: ──────────────────────────────
if Text and Text:match("^(%d+):msdl:(mp3|mp4):(%d+):(.+)$") then
  local uid, fmt, idx, cid = Text:match("^(%d+):msdl:(mp3|mp4):(%d+):(.+)$")
  if tonumber(IdUser) == tonumber(uid) then
    local stored = Redis:get(TheMero.."music:res:"..uid..":"..cid..":"..idx)
    if not stored then
      return bot.answerCallbackQuery(data.id, "⇜ انتهت صلاحية النتيجة", true)
    end
    local vid_id, title = stored:match("^([^|]+)|(.+)$")
    if not vid_id then
      return bot.answerCallbackQuery(data.id, "⇜ خطأ", true)
    end
    local link = "https://youtu.be/"..vid_id:gsub("[^%w%-_]","")
    local sfname = title:gsub("[^%w%s%-]",""):gsub("%s+","_"):sub(1,55)
    if sfname == "" then sfname = "track_"..math.random(10000,99999) end
    edit(ChatId, Msg_id, "⇜ جاري التحميل... "..(fmt=="mp3" and "🎵" or "🎬"), "md", true, false)
    if fmt == "mp3" then
      local out = "./"..sfname..".mp3"
      os.execute("timeout 150 "..YTDLP_BIN.." "..YTDLP_COOKIES
        .." '"..link.."' -f bestaudio --extract-audio --audio-format mp3"
        .." -o '"..sfname..".mp3' --no-playlist --max-filesize 50M 2>/dev/null")
      local f = io.open(out,"r")
      if f then
        f:close()
        bot.sendAudio(ChatId, Msg_id, out, "🎵 "..title, "md", nil, title)
        sleep(2); os.remove(out)
      else
        edit(ChatId, Msg_id, "⇜ لم أستطع تحميل هذا المقطع 😕", "md", true, false)
      end
    else
      local out = "./"..sfname..".mp4"
      os.execute("timeout 150 "..YTDLP_BIN.." "..YTDLP_COOKIES
        .." '"..link.."' -f 'bestvideo[ext=mp4]+bestaudio/best[ext=mp4]/best'"
        .." --merge-output-format mp4 -o '"..sfname..".mp4' --no-playlist --max-filesize 80M 2>/dev/null")
      local f = io.open(out,"r")
      if f then
        f:close()
        bot.sendVideo(ChatId, Msg_id, out, "🎬 "..title, "md")
        sleep(2); os.remove(out)
      else
        edit(ChatId, Msg_id, "⇜ لم أستطع تحميل هذا الفيديو 😕", "md", true, false)
      end
    end
  else
    bot.answerCallbackQuery(data.id, "⇜ الأمر ليس لك", true)
...
local TheMeroMsg = '*⚖┊تم إختيار الشخـص ⇜* '..ls..'\n*👩🏻‍⚖┊ليتـم الحكـم عليـه من قبـل ⇜* '..Hs..'\n\n*🎁┊وتم اضـافـة 5 نقـاط للحـاكـم*'
local ahkam_markup = bot.replyMarkup{type='inline', data={{{text='˹𓌗 قنـاة البـوت 𓌗.', url='http://t.me/'..chsource}}}}
return edit(ChatId, Msg_id, TheMeroMsg, 'md', false, false, ahkam_markup)
end
end
...
data = {
{
{text = '› ❶ ‹', data = IdUser..'/help1', style = 'primary'}, {text = '❷', data = IdUser..'/help2', style = 'primary'}, {text = '❸', data = IdUser..'/help3', style = 'primary'},
},
{
{text = '❹', data = IdUser..'/help4', style = 'primary'}, {text = '❺', data = IdUser..'/help5', style = 'primary'}, {text = '❻', data = IdUser..'/help6', style = 'primary'},
},
{
{text = 'التالي', data = IdUser..'/help2'}, {text = 'القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},
},
}
}
local TextHelp = [[
...
{
{text = 'القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},
},
}
}
local TextHelp = [[
...
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{{text = '❻', data = IdUser..'/helpp6', style = 'primary'},{text = '› ❼ ‹', data = IdUser..'/help7', style = 'primary'},{text = '❽', data = IdUser..'/help8', style = 'primary'},},
{{text = 'التالي', data = IdUser..'/help8'},{text = 'السابق', data = IdUser..'/helpp6'},},
{{text = 'القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},},
}}
local TextHelp = [[
...
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{{text = '❻', data = IdUser..'/helpp6', style = 'primary'},{text = '❼', data = IdUser..'/help7', style = 'primary'},{text = '› ❽ ‹', data = IdUser..'/help8', style = 'primary'},{text = '❾', data = IdUser..'/help9', style = 'primary'},},
{{text = 'التالي', data = IdUser..'/help9'},{text = 'السابق', data = IdUser..'/help7'},},
{{text = 'القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},},
}}
local TextHelp = [[
⤶ *اوامر التنظيف التلقائي*

⇜ تنظيف تلقائي + الوقت بالدقائق
⇜ وقت التنظيف
⇜ وقف التنظيف التلقائي
━━━━━━━━━━━
⤶ *اوامر الحماية القصوى*

⇜ الحماية القصوى
⇜ ايقاف الحماية القصوى
⇜ حالة الحماية
━━━━━━━━━━━
]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
...
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{{text = '❼', data = IdUser..'/help7', style = 'primary'},{text = '❽', data = IdUser..'/help8', style = 'primary'},{text = '› ❾ ‹', data = IdUser..'/help9', style = 'primary'},{text = '10', data = IdUser..'/help10', style = 'primary'},},
{{text = 'التالي', data = IdUser..'/help10'},{text = 'السابق', data = IdUser..'/help8'},},
{{text = 'القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},},
}}
local TextHelp = [[
⤶ *اوامر التحميل*

⇜ يوتيوب — عرض خيارات التحميل
⇜ يوت — نفس يوتيوب
⇜ انستا + رابط — تحميل انستقرام
⇜ تويتر + رابط — تحميل تويتر
⇜ تيك + رابط — تحميل تيك توك
⇜ فيس + رابط — تحميل فيسبوك
⇜ ساوند + رابط — تحميل ساوند كلاود
⇜ بحث + كلمة — بحث يوتيوب
━━━━━━━━━━━
]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
elseif Text and Text:match('(%d+)/help10') then
local UserId = Text:match('(%d+)/help10')
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{{text = '❽', data = IdUser..'/help8', style = 'primary'},{text = '❾', data = IdUser..'/help9', style = 'primary'},{text = '› ❿ ‹', data = IdUser..'/help10', style = 'primary'},},
{{text = 'السابق', data = IdUser..'/help9'},},
{{text = 'القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},},
}}
local TextHelp = [[
⤶ *اوامر التنظيف اليدوي*

⇜ امسح / مسح الميديا / تنظيف الميديا
⇜ مسح الملصقات / تنظيف الملصقات
⇜ مسح الفيديو / تنظيف الفيديو
⇜ مسح الملفات / تنظيف الملفات
⇜ مسح الصوت / تنظيف الصوت
⇜ مسح الفويسات / تنظيف الفويسات
⇜ مسح الصور / تنظيف الصور
⇜ مسح القيفات / تنظيف القيفات
━━━━━━━━━━━
⤶ *اوامر التنظيف التلقائي*
...
⇜ حالة الحماية
━━━━━━━━━━━
⤶ *اوامر الحماية المشددة*

⇜ منع التصفية / وقف منع التصفية
⇜ تفعيل الحماية 2 / تعطيل الحماية 2
⇜ تفعيل الاشعارات الذكية / تعطيل الاشعارات الذكية
⇜ قفل جمثون / فتح جمثون
━━━━━━━━━━━
]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
elseif Text and Text:match('(%d+)/help11') and not Text:match('(%d+)/help11_') then
local UserId = Text:match('(%d+)/help11')
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline', data = {
{{text = 'منع التصفية 🚬', data = IdUser..'/help11_1', style = 'primary'},{text = 'قفل الاباحي 🔞', data = IdUser..'/help11_2', style = 'primary'},},
{{text = 'الاشعارات الذكية 💡', data = IdUser..'/help11_3', style = 'primary'},},
{{text = '🔙 القائمه الرئيسية', data = IdUser..'/helpall', style = 'danger'},},
}}
local TextHelp = [[
⇜ اوامــر الحـماية المـشدده V1 💢

ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆

❶- اوامــر منـع التصـفية

❷- اوامــر قفـل الاباحـي

❸- اوامــر الاشـعـارات الـذكيـة

• اختـر احـد الازرار بالاسفـل لـ التعـرف اكثـر عـلى احـدى هـذه الاوامــر المـشدده 💢
]]
edit(ChatId,Msg_id,TextHelp,'md',true,false,reply_markup)
end

elseif Text and Text:match('(%d+)/help11_1') then
local UserId = Text:match('(%d+)/help11_1')
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline', data = {
{{text = '🔙 رجوع', data = IdUser..'/help11', style = 'danger'},},
}}
local TextHelp = [[
...
• يتميـز البـوت بمنـع المشرف الخايـن من تصفيـر اعضـاء القـروب ويقـوم بتنزيلـه
...
elseif Text and Text:match('(%d+)/help11_2') then
local UserId = Text:match('(%d+)/help11_2')
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline', data = {
{{text = '🔙 رجوع', data = IdUser..'/help11', style = 'danger'},},
}}
local TextHelp = [[
⇜  قفـل الاباحـيه المستوي الأعلـي 🔞
...
هـذه الميـزه حصريه فقـط ع سـورس ميـدو 🔅
]]
edit(ChatId,Msg_id,TextHelp,'md',true,false,reply_markup)
end

elseif Text and Text:match('(%d+)/help11_3') then
local UserId = Text:match('(%d+)/help11_3')
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline', data = {
{{text = '🔙 رجوع', data = IdUser..'/help11', style = 'danger'},},
}}
...
❹- ارجع للقروب الاساسي ثم ارسـل ( تفعيل الاشعارات الذكيه )
...
هـذه الميـزه حصريه فقـط ع سـورس ميدو 🔅
]]
edit(ChatId,Msg_id,TextHelp,'md',true,false,reply_markup)
end
...
{
{text = '❹', data = IdUser..'/help4', style = 'primary'},{text = '❺', data = IdUser..'/help5', style = 'primary'}, {text = '❻', data = IdUser..'/helpp6', style = 'primary'},
},
{
{text = '❼', data = IdUser..'/help7', style = 'primary'},{text = '❽', data = IdUser..'/help8', style = 'primary'},{text = '❾', data = IdUser..'/help9', style = 'primary'},
},
{
{text = '❿', data = IdUser..'/help10', style = 'primary'},{text = '⓫', data = IdUser..'/help11', style = 'primary'},
},
{
{text = 'اوامر القفل', data = IdUser..'/NoNextSeting', style = 'danger'}, {text = 'اوامر التعطيل', data = IdUser..'/listallAddorrem', style = 'danger'}, 
},
}
}
local TextHelp = '⤶ اهـلين فـيك بـ اوامــر روبي 🪁\n\n𝟏 ↢ اوامـر الادارة\n𝟐 ↢ اوامـر الاعدادات\n𝟑 ↢ اوامـر القفل والتعطيل\n𝟒  ↢ اوامـر التسليه\n𝟓 ↢ اوامـر الترفيه\n𝟔 ↢ اوامـر الالعاب\n𝟕 ↢ اوامـر الذكاء الاصطناعي\n𝟖 ↢ اوامـر التنظيف والحماية\n𝟗 ↢ اوامـر التحميل\n𝟏𝟎 ↢ اوامـر التنظيف\n𝟏𝟏 ↢ اوامـر الحماية المشددة\n\n⌯ Made by : [- Ahmed </>](tg://user?id='..(Sudo_Id)..')'
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
