...
local _ce_ok, CustomEmoji = pcall(require, 'libs.CustomEmoji')
if not _ce_ok then _ce_ok, CustomEmoji = pcall(require, 'libs/CustomEmoji') end
if not _ce_ok then
  CustomEmoji = {
    inject = function(x) return x end,
    plain_fmt = function(t) return {luatele='formattedText',text=t,entities={}} end,
    http_entities = function() return {} end,
    has_emoji = function() return false end,
  }
end
...
function luatele_function.keyboard_has_style(reply_markup)
  if not reply_markup or type(reply_markup) ~= 'table' then return false end
  if not reply_markup.rows then return false end
  for _, row in pairs(reply_markup.rows) do
    for _, btn in pairs(row) do
      if btn.style then return true end
    end
  end
  return false
end

function luatele_function.keyboard_to_http(reply_markup)
  local out_rows = {}
  for _, row in pairs(reply_markup.rows) do
    local http_row = {}
    for _, btn in pairs(row) do
      local http_btn = {text = btn.text}
      if btn.style then http_btn.style = btn.style end
      if btn.type then
        local t = btn.type.luatele
        if t == 'inlineKeyboardButtonTypeCallback' then
          http_btn.callback_data = luatele_function.base64_decode(btn.type.data)
        elseif t == 'inlineKeyboardButtonTypeUrl' then
          http_btn.url = btn.type.url
        elseif t == 'inlineKeyboardButtonTypeSwitchInline' then
          http_btn.switch_inline_query = btn.type.query
        elseif t == 'inlineKeyboardButtonTypeLoginUrl' then
          http_btn.login_url = {url = btn.type.url}
        elseif t == 'keyboardButtonTypeRequestLocation' then
          http_btn.request_location = true
        elseif t == 'keyboardButtonTypeRequestPhoneNumber' then
          http_btn.request_contact = true
        elseif t == 'keyboardButtonTypeRequestPoll' then
          http_btn.request_poll = {
            type = btn.type.force_quiz and 'quiz' or (btn.type.force_regular and 'regular' or nil)
          }
        end
      end
      http_row[#http_row + 1] = http_btn
    end
    out_rows[#out_rows + 1] = http_row
  end
  -- 'style' is only honored by real Telegram clients on reply-keyboard
  -- buttons (Bot API `keyboard` field) and inline buttons (`inline_keyboard`).
  -- Build the correct JSON shape for whichever markup type this is.
  if reply_markup.luatele == 'replyMarkupShowKeyboard' then
    return {
      keyboard = out_rows,
      resize_keyboard = reply_markup.resize_keyboard,
      one_time_keyboard = reply_markup.one_time,
      selective = reply_markup.is_personal
    }
  else
...
function luatele_function.replyMarkup(input)
  ...
  if input.data and (_type == 'inline' or _type == 'keyboard') then
    local _bcOk, _ = pcall(function()
      local _scope = (CurrentChatId and tostring(CurrentChatId)) or '0'
      for _, _row in pairs(input.data) do
        for _, _btn in pairs(_row) do
          if type(_btn) == 'table' and _btn.text then
            local _bc = Redis:get(TheMero.."Mero:BtnColor:".._scope..":".._btn.text)
            if not _bc then
              _bc = Redis:get(TheMero.."Mero:BtnColor:global:".._btn.text)
            end
            if _bc then
              if _bc == 'default' then
                _btn.style = nil
...
function luatele_function.getParseMode(parse_mode)
  if parse_mode then
    ...
    elseif mode == 'mdv2' or mode == 'markdownv2' then
      return {
        luatele = 'textParseModeMarkdownV2'
      }
...
local function strip_custom_emoji_entities(result)
  if result and type(result.entities) == 'table' then
    local filtered = {}
    for _, e in ipairs(result.entities) do
      if not (e.type and e.type.luatele == 'textEntityTypeCustomEmoji') then
...
function luatele_function.sendMessage(chat_id, reply_to_message_id, input_message_content, parse_mode, disable_notification, from_background, reply_markup)
  if luatele_function.keyboard_has_style(reply_markup) then
    local raw_text = (input_message_content.text and input_message_content.text.text)
                  or (input_message_content.caption and input_message_content.caption.text)
                  or ''
    local http_keyboard = luatele_function.keyboard_to_http(reply_markup)
    local pm = (parse_mode == 'md') and 'markdown' or (parse_mode == 'mdv2' or parse_mode == 'markdownv2') and 'MarkdownV2' or (parse_mode == 'html' and 'html') or nil
    local send_text, send_ents, use_pm = raw_text, nil, pm
    if pm then
      local fmt = luatele_function.parseTextEntities(raw_text, parse_mode)
      if fmt and fmt.text then
        local pt, ents = CustomEmoji.fmt_to_http_entities(fmt)
        if ents and #ents > 0 then
          send_text, send_ents, use_pm = pt, ents, nil
        end
      end
    else
      local ce = CustomEmoji.http_entities(raw_text)
      if #ce > 0 then send_ents = ce end
    end
    local url_str = 'https://api.telegram.org/bot'..Token..'/sendMessage?chat_id='..tostring(chat_id)..'&text='..URL.escape(send_text)..'&disable_web_page_preview=true'
    if use_pm then url_str = url_str..'&parse_mode='..use_pm end
    if send_ents and #send_ents > 0 then url_str = url_str..'&entities='..URL.escape(JSON.encode(send_ents)) end
    if reply_to_message_id and reply_to_message_id ~= 0 then
      local http_reply_id = math.floor(tonumber(reply_to_message_id) / 1048576)
      if http_reply_id < 1 then http_reply_id = tonumber(reply_to_message_id) end
      url_str = url_str..'&reply_to_message_id='..tostring(http_reply_id)
    end
    url_str = url_str..'&reply_markup='..URL.escape(JSON.encode(http_keyboard))
    local ok, code = https.request(url_str)
    Redis:set(TheMero..'debug:last:sendMsg', tostring(code)..':'..tostring(ok):sub(1,200))
    return ok, code
  end
  ...
  if text then
    if parse_mode then
      local result = strip_custom_emoji_entities(luatele_function.parseTextEntities(text, parse_mode))
      ...
      message[#message + 1] = text
      for i = 1, #message do
        local plain = CustomEmoji.plain_fmt(message[i])
        if input_message_content.text and input_message_content.text.text then
          if plain then
            luatele_body.input_message_content.text = plain
          else
        ...
        elseif input_message_content.caption and input_message_content.caption.text then
          if plain then
            luatele_body.input_message_content.caption = plain
          else
...
function luatele_function.editMessageText(chat_id, message_id, text, parse_mode, disable_web_page_preview, clear_draft, reply_markup)
  if luatele_function.keyboard_has_style(reply_markup) then
    local http_keyboard = luatele_function.keyboard_to_http(reply_markup)
    local pm = (parse_mode == 'md') and 'markdown' or (parse_mode == 'mdv2' or parse_mode == 'markdownv2') and 'MarkdownV2' or (parse_mode == 'html' and 'html') or nil
    local http_message_id = math.floor(tonumber(message_id) / 1048576)
    if http_message_id < 1 then http_message_id = tonumber(message_id) end
    local raw_edit_text = tostring(text or '')
    local send_text, send_ents, use_pm = raw_edit_text, nil, pm
    if pm then
      local fmt = luatele_function.parseTextEntities(raw_edit_text, parse_mode)
      if fmt and fmt.text then
        local pt, ents = CustomEmoji.fmt_to_http_entities(fmt)
        if ents and #ents > 0 then
          send_text, send_ents, use_pm = pt, ents, nil
      ...
      local ce = CustomEmoji.http_entities(raw_edit_text)
      if #ce > 0 then send_ents = ce end
    end
    local url_str = 'https://api.telegram.org/bot'..Token..'/editMessageText?chat_id='..tostring(chat_id)..'&message_id='..tostring(http_message_id)..'&text='..URL.escape(send_text)..'&disable_web_page_preview=true'
    if use_pm then url_str = url_str..'&parse_mode='..use_pm end
    if send_ents and #send_ents > 0 then url_str = url_str..'&entities='..URL.escape(JSON.encode(send_ents)) end
    url_str = url_str..'&reply_markup='..URL.escape(JSON.encode(http_keyboard))
    local ok, code = https.request(url_str)
    Redis:set(TheMero..'debug:last:editMsg', tostring(code)..':'..tostring(ok):sub(1,200))
    return ok, code
  end
  local luatele_body = {
  ...
  if parse_mode then
    luatele_body.input_message_content.text = strip_custom_emoji_entities(luatele_function.parseTextEntities(text, parse_mode))
  else
    local plain = CustomEmoji.plain_fmt(tostring(text or ''))
    if plain then luatele_body.input_message_content.text = plain end
  end
  return function_core.run_table(luatele_body)
end
function luatele_function.editMessageCaption(chat_id, message_id, caption, parse_mode, reply_markup)
  ...
  if parse_mode then
      luatele_body.caption = strip_custom_emoji_entities(luatele_function.parseTextEntities(text,parse_mode))
  end
  return function_core.run_table(luatele_body)
end
...
function luatele_function.setChatDraftMessage(chat_id, reply_to_message_id, text, parse_mode, disable_web_page_preview, clear_draft)
  ...
  if parse_mode then
      luatele_body.draft_message.input_message_text.text = strip_custom_emoji_entities(luatele_function.parseTextEntities(text, parse_mode))
  end
  return function_core.run_table(luatele_body)
end
...
function luatele_function.processChatJoinRequest(chat_id, user_id, approve)
  return function_core.run_table{
    luatele = 'processChatJoinRequest',
    chat_id = chat_id,
    user_id = tonumber(user_id),
    approve = approve
  }
